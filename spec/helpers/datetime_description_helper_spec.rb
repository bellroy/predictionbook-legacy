...
describe DatetimeDescriptionHelper do
  ...
  describe 'regression test' do
    it 'should not crash on 100 years from now' do
      lambda { time_in_words_with_context(100.years.from_now) }.should_not raise_error(RangeError)
    end
    it 'should not crash on 30 years from now' do
      lambda { time_in_words_with_context(30.years.from_now) }.should_not raise_error(NoMethodError)
    end
  end
